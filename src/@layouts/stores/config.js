...
export const useLayoutConfigStore = defineStore('layoutConfig', () => {
  ...
  const navbarType = computed(() => {
    return isLessThanOverlayNavBreakpoint.value ? NavbarType.Sticky : layoutConfig.navbar.type
  })



  // 👉 Mobile Screen Detection using VueUse
  const { width: windowWidth } = useWindowSize()
  const isMobileScreen = computed(() => windowWidth.value <= 720)
  ...
  const _layoutClasses = computed(() => {
    ...
    return [
      ...
      { 'layout-overlay-nav': isLessThanOverlayNavBreakpoint.value },
      { 'window-scrolled': unref(windowScrollY) },
      { 'navbar-bottom-fixed': isMobileScreen.value },
      route.meta.layoutWrapperClasses ? route.meta.layoutWrapperClasses : null,
    ]
  })
